def shuffle(s):
    ...
    return [s[i >> 1] if i % 2 == 0 else s[(i + len(s)) >> 1] for i in range(len(s))]
...
def deep_map(f, s):
    ...
    def helper(l):
        for i in range(len(l)):
            l[i] = helper(l[i]) if (type(l[i]) == list) else f(l[i])
        return l
...
def balanced(m):
    ...
    return total_mass(end(left(m))) * length(left(m)) == total_mass(end(right(m))) * length(right(m)) and (is_planet(end(left(m))) or balanced(end(left(m)))) and (is_planet(end(right(m))) or balanced(end(right(m))))
...
def max_path_sum(t):
    ...
    return label(t) + max([max_path_sum(branch) for branch in branches(t)] + [0])
