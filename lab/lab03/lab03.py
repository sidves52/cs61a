...
def ten_pairs(n):
    ...
    pairs_of_fives = lambda x: round((x * (x + 1)) / 2) # sum of consecutive digits

    count = 0
    count += count_digit(n, 1) * count_digit(n, 9)
    count += count_digit(n, 2) * count_digit(n, 8)
    count += count_digit(n, 3) * count_digit(n, 7)
    count += count_digit(n, 4) * count_digit(n, 6)
    count += pairs_of_fives(count_digit(n, 5) - 1)
