...
def buy(fruits_to_buy: list[str], prices: dict[str, int], total_amount: int) -> None:
    ...
    def add(fruits: list[str], amount: int, cart: str) -> None:
        if fruits == [] and amount == 0:
            ...
            fruit = fruits[0]
            price = prices[fruit]
            for num_to_buy in range(1, (amount // price)+1):
                # Hint: The display function will help you add fruit to the cart.
                add(fruits[1:], amount - (price * num_to_buy), cart + display(fruit, num_to_buy))
...
def closer_city(lat, lon, city_a, city_b):
    ...
    if (distance(coords, city_a) < distance(coords, city_b)):
